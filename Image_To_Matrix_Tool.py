...
def resize_and_convert_to_gray(input_paths, output_folder):
    for input_path in input_paths:
        try:
            ...
            dane.append("]]")
            save_in_file(dane, output_folder, filename)
...
def save_in_file(dane, output_folder, filename):
    sciezka_pliku = output_folder + "/{}_32x16_matrix.txt".format(filename)
    with open(sciezka_pliku, 'w') as plik:
        for linia in dane:
            plik.write(linia + '\n')
    print("Dane zostały zapisane w pliku:", sciezka_pliku)
